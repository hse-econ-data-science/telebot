# Работу выполнили студенты:
    # Кондратьева Надежда (БЭК201)
    # Никитюк Михаил (БЭК203)
    # Гаврилова Анастасия (БЭК201)
...
bot = telebot.TeleBot("1797228419:AAHhAU03ULrcYmA49iLtOAPfmuoRM6qDEqo")
...
def callback_worker_izbrannoe(call):
    if call.text == 'Просмотреть полный список':
        try:
            show_plans(call)
            bot.send_message(call.chat.id, text='Выполнил показ всего избранного')
            bot.send_message(call.chat.id, text='Чем помочь еще?')
            send_izbrannoe(call)
        except:
...
def delete_all_plans(msg):
    with sqlite3.connect('pref.db') as con:
        ...
        con.commit()
    bot.send_message(msg.chat.id, 'Удалены все избранные рецепты. Хорошего отдыха!')
    send_izbrannoe(msg)
